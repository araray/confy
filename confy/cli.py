...
@click.option("--no-dotenv",       is_flag=True, default=False,
...
def cli(ctx, file_path, prefix, overrides, defaults, mandatory, dotenv_path, no_dotenv):
    ...
    if defaults:
        try:
            ...
            click.secho(f"Error: Defaults file not found: {defaults}", fg="red", err=True)
            ctx.exit(1)
        except json.JSONDecodeError as e:
            click.secho(f"Error parsing defaults file {defaults}: {e}", fg="red", err=True)
            ctx.exit(1)
    ...
    if overrides:
        for pair in overrides.split(","):
            if ":" in pair:
                ...
                click.secho(f"Warning: Malformed override '{pair}'. Use 'key:json_value' format.", fg="yellow", err=True)
...
def set(ctx, key, value):
    ...
    try:
        if ext == ".toml":
            ...
            click.secho(f"Error: Unsupported file type for set: {ext}", fg="red", err=True)
            ctx.exit(1)
    except FileNotFoundError:
...
def search(ctx, key_pat, val_pat, ignore_case):
    ...
    if not (key_pat or val_pat):
        click.secho("Error: Please supply --key and/or --val pattern.", fg="red", err=True)
        ctx.exit(1)
...
@cli.command()
@click.option("--to", "fmt", type=click.Choice(["json","toml"], case_sensitive=False),
              required=True, help="Format to convert to (json or toml).")
...
def convert(ctx, fmt, out_file):
    ...
    if out_file:
        try:
            ...
            click.secho(f"Error writing output to file {out_file}: {e}", fg="red", err=True)
            ctx.exit(1)
    else:
