"""Shared token-counting utilities for the Wairu ecosystem.

The public ``count_tokens`` helper prefers tiktoken when it is installed and
falls back to a deterministic character-ratio estimate when it is not. This
keeps token budgeting available in minimal installs while giving precise
counts in richer environments.
"""
...
def count_tokens(text: str | None, model: str | None = None) -> int:
    ...
    Args:
        text: Text to count. ``None`` and empty strings count as zero.
        model: Optional model name used to choose a model-specific tokenizer.
...
def get_counter(model: str | None = None) -> TokenCounter:
    ...
    The returned counter uses tiktoken when importable and falls back to
    ``EstimateCounter`` when tiktoken or its selected encoding cannot load.
    """
    encoding = _load_encoding(model)
    if encoding is None:
